...
## ER Graph
gg=random.graph.game(n=Nv, (K*Nv)/2, type="gnm", directed=FALSE)
...
df = tibble(tau=seq(10^-2,10^0,by=0.01))
```

> Plot probability distribution of eigenvalues

```{r}

bw  = 4
df1 = bin.eigen(Emean, x.min=1e-2, x.max=1e2, steps=1, group="A")
df1 = smooth.bin.eigen(df1, bandwidth=bw)

pt1 = ggplot(df1, aes(x,y,group=group))+
...
dat = data.frame(x=df$tau, y1=y1, y2=log.y2)
...
```{r}
tt.steps = 1e-2
tt      = tibble(tau=seq(1e-2,1e0, tt.steps))
...
pt5 <- ggplot(df.scan, aes(x,y)) +
  ...
  #geom_line(data=lrg.ann2$df, aes(x=new_t,y=Loss.new), color="red")+
  theme_light() 
...
R = real.LRG(e=E[[1]], v=E[[2]], vinv=E[[3]], L=L, t=tau.best1, gg=gg, method="eigen", order=1)
table(R$supernodes)
