...
L=vector(mode="list", length=length(deg.tab))
names(L) = names(deg.tab)
k=1
for( i in names(deg.tab) ){ L[[k]] = sum(ee[names(ee)==i])/deg.tab[k]; k=k+1; }

L = L %>% 
    unlist(.) %>%
    data_cdf_probs(.)

dx = data.frame(x=as.numeric(names(L)),
                y=as.vector(L))
```

```{r}
plot(xy.coords(x=log(dx$x), y=log(dx$y)), xlab=c("log(K)"), ylab=c("P(E)"), type="l")
```
...
#if( length(tau.x) >= 1 ){ tau = tau.x[1] } 

tps = turing.points(pt4, print=0)

#ii  = max(which(tps$tp$tp.x_axis<tau))
#if( length(ii)==0 ){ ii=1; }

#t.lower = ifelse(ii>1, tps$tp$tp.min.x_axis[ii-1], tps$tp$tp.min.x_axis[1])
##tps.set    = ifelse(tps$tp$tp.x_axis<tau, abs(tps$tp$tp.x_axis-tau), NA)
##indx.close = which.min(tps.set)
##t.lower    = tps$tp$tp.x_axis[indx.close]
...
##test
#t.upper = tau.x[1] #tps$tp$tp.x_axis[4] #0.090
#.lower = 1/Emean[1] #0.01 #tps$tp$tp.x_axis[1] #0.060
...
R = real.LRG(e=Emean, v=Evec, vinv=Evinv, L=L, t=tau.best1, gg=gg, method="eigen", order=1)
table(R$supernodes)
